...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Ensure that storedUserData exists before accessing properties
    if (storedUserData && storedUserData.role) {
      setUserData(storedUserData);
      setUser(true); 
      // navigate(`/${storedUserData.role}`);
    }
  }, [navigate]);
    ...
    <div className="flex flex-col h-screen">
      <div className="flex items-center justify-between bg-[#3eabd6] mx-4 rounded mt-1 p-4">
        ...
        <div className="flex items-center">
          {user ? (
            <div className="flex items-center">
              <p className="mr-2">Hello {userData && userData.id}</p> 
      ...
      <div className="flex-grow">
        <Routes>
          ...
          <Route path='/agent/chat/:id' element={<Agent />} />
